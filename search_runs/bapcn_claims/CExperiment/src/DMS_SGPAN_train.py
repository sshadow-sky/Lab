...
from experiment_model import BAPCN as DMS_SGPAN
...
def _state_digest(model):
    ...
    digest.update(str(model.subject_grl.iter_num).encode())
    return digest.hexdigest()


def _save_experiment_checkpoint(path, model, optimizer, args, split, params, epoch, tag):
    torch.save({
        "model_state": model.state_dict(), "optimizer_state": optimizer.state_dict(),
        "net_params": dict(params, DEVICE=str(params["DEVICE"])),
        "prototype_space": args.prototype_space, "profile": args.profile,
        "ablation": args.ablation,
        "split": split, "epoch": int(epoch), "checkpoint_tag": tag,
        "grl_iter": int(model.subject_grl.iter_num), "model_digest": _state_digest(model),
    }, path)


def _write_test_metrics(output_dir, tag, rows, epoch):
    path = Path(output_dir) / "metrics" / f"test_{tag}.csv"
    with path.open("w", newline="", encoding="utf-8") as stream:
        writer = csv.DictWriter(stream, fieldnames=("subject_id", "accuracy", "macro_f1"))
        writer.writeheader()
        for subject_index, metric in rows:
            writer.writerow({"subject_id": int(subject_index) + 1,
                             "accuracy": float(metric["acc"]), "macro_f1": float(metric["macro-f1"])})
    summary = {"checkpoint_epoch": int(epoch), "checkpoint_tag": tag,
               "classifier": "original_final_classifier", "metrics_units": "fraction",
               "subject_std_ddof": 0, "subjects": [int(sid) + 1 for sid, _ in rows]}
    for name, key in (("accuracy", "acc"), ("macro_f1", "macro-f1")):
        values = [float(metric[key]) for _, metric in rows]
        summary[name] = {"mean": float(np.mean(values)), "std": float(np.std(values))}
    _write_json(path.with_suffix(".json"), summary)


def _export_saved_checkpoints(args, output_dir, subjects):
    if args.skip_export:
        return
    from evidence import export_checkpoint
    names = {"warmup15": "epoch_015.pt", "final": "final.pt", "best": "best.pt"}
    for tag in args.checkpoint_tags:
        checkpoint = Path(output_dir) / "checkpoints" / names[tag]
        if not checkpoint.is_file():
            raise FileNotFoundError(f"Checkpoint unavailable for export: {checkpoint}")
        result = export_checkpoint(
            checkpoint, subjects, Path(output_dir) / "evidence" / tag,
            device=args.device, batch_size=args.export_batch_size,
        )
        print(f"BAPCN {tag} evidence: {result}", flush=True)


def _subjects_from_arrays(*cohorts):
    subjects = {}
    for features, labels, subject_ids in cohorts:
        for sid in np.unique(subject_ids):
            mask = subject_ids == sid
            class_labels = labels[mask]
            if class_labels.ndim > 1:
                class_labels = class_labels.argmax(axis=-1)
            current = (features[mask], class_labels.astype(np.int64, copy=False))
            key = int(sid) + 1
            if key in subjects:
                if not all(np.array_equal(left, right) for left, right in zip(subjects[key], current)):
                    raise ValueError(f"Inconsistent repeated subject {key}")
            else:
...
def _print_test_subject_summary(args, test_subject_records):
    if len(test_subject_records) == 0:
        print("DMS_SGPAN test subject summary: no finished test subject records")
        return

    print("\nDMS_SGPAN test subject summary")
    print(f"DMS_SGPAN per-test-subject records: {len(test_subject_records)}")
...
def _train_one_round(
    ...
    model = DMS_SGPAN(model_params, prototype_space=args.prototype_space, ablation=args.ablation).to(device)
    output_dir = Path(output_dir)
    for name in ("checkpoints", "metrics"):
        (output_dir / name).mkdir(parents=True, exist_ok=True)
    fixed_final = args.profile["protocol"] == "loso"
    manifest = {
        "profile": args.profile, "split": split,
        "net_params": dict(model_params, DEVICE=str(device)),
        "prototype_space": args.prototype_space, "initial_digest": _state_digest(model),
        "ablation": args.ablation,
        "alignment_space": "fused" if args.ablation == "fused_alignment" else "band",
        "reliability_formula": "(P*margin*entropy)^(1/3)" if args.ablation == "no_agreement" else "(P*margin*entropy*agreement)^(1/4)",
        "model_source_sha256": hashlib.sha256(Path(__file__).with_name("baseline_model.py").read_bytes()).hexdigest(),
        "torch_version": str(torch.__version__), "cuda_version": torch.version.cuda,
        "device": str(device), "status": "training",
        "selection_policy": "fixed_final_epoch" if fixed_final else "validation_macro_f1_after_warmup",
    }
    _write_json(output_dir / "manifest.json", manifest)

    reference = None
    if getattr(args, "baseline_dir", None):
        reference = json.loads((Path(args.baseline_dir) / "manifest.json").read_text(encoding="utf-8"))
        if reference.get("prototype_space") != "band" or reference.get("ablation", "baseline") != "baseline":
            raise ValueError("The reference must be an unablated band baseline")
        for key in ("split", "net_params", "initial_digest", "model_source_sha256", "torch_version", "cuda_version"):
            if manifest[key] != reference.get(key):
                raise ValueError(f"Baseline reference differs in {key}; training stopped before epoch 1")
        expected_profile, actual_profile = dict(reference["profile"]), dict(args.profile)
        expected_profile.pop("dataset_path", None)
        actual_profile.pop("dataset_path", None)
        if expected_profile != actual_profile or reference.get("status") != "complete":
            raise ValueError("Baseline reference profile differs or training is incomplete")
    ...
    if fixed_final:
        print(f"BAPCN LOSO checkpoint: fixed epoch {args.epochs}; target labels are not used for selection.")
    else:
        print(
    ...
    for epoch in range(args.epochs):
        ...
        # Save only here; evidence passes are deferred until training has finished.
        if epoch_number == warmup_epochs:
            _save_experiment_checkpoint(
                output_dir / "checkpoints" / f"epoch_{epoch_number:03d}.pt",
                model, optimizer, args, split, model_params, epoch_number, "warmup15",
            )
            manifest["warmup_digest"] = _state_digest(model)
            _write_json(output_dir / "manifest.json", manifest)
            if reference is not None and manifest["warmup_digest"] != reference.get("warmup_digest"):
                raise ValueError("Epoch-15 state differs from baseline; stopped before adaptation")
        ...
        eval_metric = None
        if should_eval and not fixed_final:
            ...
            if epoch_number < selection_start_epoch:
                ...
                _save_experiment_checkpoint(
                    output_dir / "checkpoints" / "best.pt", model, optimizer,
                    args, split, model_params, epoch_number, "best",
                )
                print(
        ...
        history_row = {"epoch": epoch_number, **diag_means,
                       "validation_accuracy": "" if eval_metric is None else float(eval_metric["acc"]),
                       "validation_macro_f1": "" if eval_metric is None else float(eval_metric["macro-f1"])}
        with history_path.open("w" if epoch == 0 else "a", newline="", encoding="utf-8") as stream:
            writer = csv.DictWriter(stream, fieldnames=list(history_row))
            if epoch == 0:
                writer.writeheader()
            writer.writerow(history_row)

    _save_experiment_checkpoint(
        output_dir / "checkpoints" / "final.pt", model, optimizer,
        args, split, model_params, int(args.epochs), "final",
    )
    manifest["final_digest"] = _state_digest(model)

    ckpt_path = Path(output_dir) / f"checkpoint-best{metric_key}"
    if fixed_final:
        selected_epoch = int(args.epochs)
    elif best_checkpoint_saved and ckpt_path.exists():
    ...
    _write_test_metrics(output_dir, "final" if fixed_final else "best", test_subject_metrics, selected_epoch)
    if not fixed_final:
        state = torch.load(output_dir / "checkpoints" / "final.pt", map_location=device, weights_only=False)
        model.load_state_dict(state["model_state"])
        model.subject_grl.iter_num = int(state["grl_iter"])
        final_rows = []
        for subject_index, subject_dataset in test_subject_datasets:
            print(f"BAPCN final checkpoint test subject {_format_subject_indexes([subject_index])}")
            metric = _evaluate(model, subject_dataset, args.metrics, device, batch_size=args.batch_size)
            final_rows.append((subject_index, metric))
        _write_test_metrics(output_dir, "final", final_rows, int(args.epochs))
    manifest["selected_epoch"] = int(selected_epoch)
...
def main(args):
    ...
    if str(train_cfg.get("optimizer", "adamw")).lower() != args.profile["optimizer"] or float(train_cfg.get("weight_decay", 1e-4)) != args.profile["weight_decay"]:
        raise ValueError("LibEER optimizer/weight_decay differ from the frozen B configuration")
    if int(train_cfg.get("eval_interval", 1)) != 1:
        raise ValueError("CExperiment requires B's validation interval of one epoch")

    if args.setting is not None:
        setting = preset_setting[args.setting](args)
    else:
        setting = set_setting_by_args(args)
    expected_split_type = "leave-one-out" if args.profile["protocol"] == "loso" else "train-val-test"
    if setting.split_type != expected_split_type or setting.experiment_mode != "subject-independent":
        raise ValueError(f"Expected subject-independent {expected_split_type}, got {setting.split_type}")
    ...
    for rridx, (data_i, label_i) in enumerate(zip(data, label), 1):
        ...
        for ridx, (train_indexes, test_indexes, val_indexes) in enumerate(zip(tts["train"], tts["test"], tts["val"]), 1):
            ...
            # Native LOSO has no validation split. Its held-out features are the
            # unlabeled target stream; the fixed final checkpoint needs no target-label selection.
            if len(val_data_keep) == 0:
                if setting.split_type == "leave-one-out":
                    ...
                    print(
                        "BAPCN transductive LOSO: using held-out subject features as target; "
                        "labels are reserved for evaluation after fixed-epoch training."
                    )
                else:
            ...
            model_params = {
                ...
                "dropout": float(params_cfg.get("dropout", 0.4)),
                "temperature": float(params_cfg.get("temperature", 0.2)),
                "ugfcda_warmup_epochs": int(
                    train_cfg.get(
                        "ugfcda_warmup_epochs",
                        params_cfg.get("ugfcda_warmup_epochs", 15),
                    )
                ),
                "ugfcda_eps": float(params_cfg.get("ugfcda_eps", 1e-6)),
                "ugfcda_reliability_threshold": float(
                    params_cfg.get("ugfcda_reliability_threshold", 0.95)
                ),
                "ugfcda_proto_align_weight": float(
                    params_cfg.get("ugfcda_proto_align_weight", 0.5)
                ),
                ...
                "w_gcl": float(train_cfg.get("loss_gcl", 0.25)),
                "w_align": float(train_cfg.get("loss_align", 0.2)),
                "w_orth": float(train_cfg.get("loss_orth", 0.5)),
                "w_subject": float(train_cfg.get("loss_subject", 0.25)),
            }

            split = {
                "name": f"subject_{int(test_indexes[0]) + 1:02d}" if setting.split_type == "leave-one-out" else "tvt",
                "source": [int(sid) + 1 for sid in train_indexes],
                "target": [int(sid) + 1 for sid in effective_val_indexes],
                "test": [int(sid) + 1 for sid in test_indexes],
            }
            output_dir = Path(args.output_dir) / split["name"]
            subjects = _subjects_from_arrays((source_x, source_y, source_sid),
                                             (target_x, target_y, target_sid),
                                             (test_x, test_y, test_sid))
            if args.export_only:
                manifest_path = output_dir / "manifest.json"
                if not manifest_path.is_file():
                    raise FileNotFoundError(f"No existing training manifest: {manifest_path}")
                manifest = json.loads(manifest_path.read_text(encoding="utf-8"))
                for key, value in (("split", split), ("profile", args.profile), ("ablation", args.ablation),
                                   ("prototype_space", args.prototype_space)):
                    if manifest.get(key) != value:
                        raise ValueError(f"Export configuration differs from saved {key}: {manifest_path}")
                if manifest.get("status") not in ("trained", "complete"):
                    raise ValueError("Checkpoint export requires completed training")
                required = [output_dir / "checkpoints" / name
                            for name in ("epoch_015.pt", "best.pt", "final.pt")]
                required.extend(output_dir / "metrics" / name
                                for name in ("history.csv", "test_best.csv", "test_final.csv"))
                for path in required:
                    if not path.is_file():
                        raise FileNotFoundError(f"Completed training artifact missing: {path}")
                _export_saved_checkpoints(args, output_dir, subjects)
                manifest["status"] = "complete"
                manifest["evidence_exported"] = True
                _write_json(manifest_path, manifest)
                continue
            if output_dir.exists() and (not output_dir.is_dir() or any(output_dir.iterdir())):
                raise FileExistsError(f"Refusing to overwrite existing training output: {output_dir}")
            ...
            manifest = json.loads(manifest_path.read_text(encoding="utf-8"))
            manifest["status"] = "complete"
            manifest["evidence_exported"] = not args.skip_export
            _write_json(manifest_path, manifest)
    ...
    if args.export_only:
        print("BAPCN checkpoint re-export completed.")
        return
    ...
    if setting.experiment_mode == "subject-dependent":
        ...
        if len(test_subject_records) > 1:
            print(
                "DMS_SGPAN final result_log uses per-test-subject metrics "
...
if __name__ == "__main__":
    raise SystemExit("Launch this experiment with ../run.sh or ../run.py")
