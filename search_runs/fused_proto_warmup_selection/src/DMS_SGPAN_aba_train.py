...
from warmup_export import export_warmup_snapshot


param_path = "config/model_param/DMS_SGPAN.yaml"


def _write_warmup_metrics(path, dataset, variant, metrics, epoch):
    with (Path(path) / "warmup_test_metrics.csv").open("w", newline="", encoding="utf-8") as stream:
        writer = csv.writer(stream)
        writer.writerow(["dataset", "variant", "checkpoint_epoch", *metrics.keys()])
        writer.writerow([dataset, variant, int(epoch), *[float(value) for value in metrics.values()]])


def _write_final_metrics(path, dataset, variant, metrics, epoch):
    with (Path(path) / "final_test_metrics.csv").open("w", newline="", encoding="utf-8") as stream:
        writer = csv.writer(stream)
        writer.writerow(["dataset", "variant", "checkpoint_epoch", *metrics.keys()])
        writer.writerow([dataset, variant, int(epoch), *[float(value) for value in metrics.values()]])
...
def _print_test_subject_summary(args, test_subject_records):
    run_name = getattr(args, "model", "DMS_SGPAN")
    if len(test_subject_records) == 0:
        print(f"{run_name} test subject summary: no finished test subject records")
        return

    print(f"\n{run_name} test subject summary")
    print(f"{run_name} per-test-subject records: {len(test_subject_records)}")
...
def _train_one_round(
    ...
    model_cls=DMS_SGPAN,
    run_name="DMS_SGPAN",
    checkpoint_selection_start_epoch=None,
):
    setup_seed(args.seed)
    model = model_cls(model_params).to(device)
    if model.normalization_mode == "standard":
        print(
            f"{run_name} normalization: source-training global Z-score at input; "
            "parameter-free LayerNorm at matched hidden positions."
        )
    else:
        print(
            f"{run_name} normalization: subject-wise Min-Max at input; "
            "subject-wise Z-score at hidden positions."
        )
    ...
    alignment_warmup_epochs = max(0, int(model.ugfcda_warmup_epochs))
    if checkpoint_selection_start_epoch is None:
        selection_start_epoch = max(1, alignment_warmup_epochs + 1)
    else:
        selection_start_epoch = max(1, int(checkpoint_selection_start_epoch))
    if int(args.epochs) < selection_start_epoch:
        raise ValueError(
            f"{run_name} requires at least {selection_start_epoch} epochs for "
            f"checkpoint selection, but args.epochs={args.epochs}. "
            "No checkpoint would be eligible for selection."
        )
    print(
        f"{run_name} schedule: prototype-alignment warmup={alignment_warmup_epochs} epochs; "
    ...
    for epoch in range(args.epochs):
        ...
        print(
            ...
            f"align_sample={diag_means['sample_align_loss']:.4f} "
            f"align_proto={diag_means['prototype_align_loss']:.4f} "
        ...
        if epoch_number == alignment_warmup_epochs:
            # Preserve the last representation before PConA is activated.
            save_state(output_dir, model, optimizer, epoch_number, metric="warmup15", state="")
            # Diagnostics must not advance stateful GRL schedules.  Otherwise
            # the extra no-grad passes change the post-warm-up optimization.
            grl = getattr(model, "subject_grl", None)
            grl_iter_num = getattr(grl, "iter_num", None)
            try:
                warmup_metrics = _evaluate(
                    model,
                    test_dataset,
                    args.metrics,
                    device,
                ...
                _write_warmup_metrics(output_dir, args.dataset, run_name, warmup_metrics, epoch_number)
                export_warmup_snapshot(
                    output_dir=output_dir,
                    model=model,
                    source_loader=source_loader,
                    target_loader=target_loader,
                    device=device,
                    configured_threshold=float(model.ugfcda_reliability_threshold),
                    dataset=args.dataset,
                    variant=run_name,
                    epoch=epoch_number,
                )
            finally:
                if grl_iter_num is not None:
                    grl.iter_num = grl_iter_num
        ...
        if should_eval:
            ...
            if epoch_number < selection_start_epoch:
                print(
                    f"{run_name} validation at epoch {epoch_number}: "
                    f"{metric_key}={float(eval_metric[metric_key]):.4f}; "
                    f"the shared checkpoint-selection window is not active, so this checkpoint is not eligible "
                ...
                print(
                    f"{run_name} selected new best checkpoint at epoch "
    ...
    if best_checkpoint_saved and ckpt_path.exists():
        ...
        print(
            f"{run_name} loading best eligible checkpoint: "
            f"epoch={selected_epoch}, {metric_key}={best_metric:.4f}"
        )
    else:
        raise RuntimeError(
            f"{run_name} did not create an eligible checkpoint. "
            f"selection_start_epoch={selection_start_epoch}, epochs={args.epochs}, "
    ...
    _write_final_metrics(output_dir, args.dataset, run_name, test_metric, selected_epoch)
    test_subject_metrics = []
    for subject_index, subject_dataset in test_subject_datasets:
        print(f"{run_name} evaluating test subject {_format_subject_indexes([subject_index])}")
...
def run_experiment(
    ...
    for rridx, (data_i, label_i) in enumerate(zip(data, label), 1):
        ...
        for ridx, (train_indexes, test_indexes, val_indexes) in enumerate(zip(tts["train"], tts["test"], tts["val"]), 1):
            ...
            print(
                f"{run_name} round {ridx}: current test subject(s) "
            ...
            if len(val_data_keep) == 0:
                if setting.split_type == "leave-one-out":
                    ...
                    print(
                        f"{run_name} non-strict LOSO compatibility: val split is empty; "
            ...
            print(
                f"{run_name} round {ridx}: samples source={len(source_x)} "
            ...
            model_params = {
                ...
                "ugfcda_reliability_threshold": float(params_cfg.get("ugfcda_reliability_threshold", 0.95)),
                "ugfcda_proto_align_weight": float(params_cfg.get("ugfcda_proto_align_weight", 0.5)),
            ...
            if ablation_variant is not None:
                model_params["ablation_variant"] = str(ablation_variant)
            if ablation_variant == "standard_norm":
                source_flat = source_x.reshape(len(source_x), -1).astype(np.float32)
                norm_eps = float(model_params["ssbn_eps"])
                model_params["standard_norm_mean"] = source_flat.mean(axis=0).astype(np.float32)
                model_params["standard_norm_std"] = np.maximum(
                    source_flat.std(axis=0),
                    norm_eps,
                ).astype(np.float32)

            shared_selection_start_epoch = int(model_params["ugfcda_warmup_epochs"]) + 1
            output_dir = make_output_dir(args, run_name)
            round_metric, round_subject_metrics = _train_one_round(
                ...
                model_cls=model_cls,
                run_name=run_name,
                checkpoint_selection_start_epoch=shared_selection_start_epoch,
            )
            ...
            print(
                f"{run_name} round {ridx}: test subject(s) "
    ...
    if len(best_metrics) == 0:
        print(f"{run_name} finished without valid round metrics; check split/loader diagnostics above.")
        return
    ...
    if setting.experiment_mode == "subject-dependent":
        ...
        if len(test_subject_records) > 1:
            print(
                f"{run_name} final result_log uses per-test-subject metrics "
