...
def _normalize_subject_ids(subject_ids: List[int], num_subjects: int, name: str) -> List[int]:
    """Validate and normalize a subject-index list."""
    ids = [int(x) for x in subject_ids]
    if len(ids) == 0:
        raise ValueError(f'{name} split is empty.')
    if len(set(ids)) != len(ids):
        raise ValueError(f'{name} split contains duplicated subject ids: {ids}')
    invalid = [x for x in ids if x < 0 or x >= num_subjects]
    if invalid:
        raise ValueError(
            f'{name} split contains invalid subject ids {invalid}; '
            f'valid range is 0..{num_subjects - 1}.'
        )
    return ids


def _dataset_split_key(dataset_name: str) -> str:
    """Map dataset aliases to split-preset keys."""
    key = str(dataset_name).strip().lower().replace('-', '_')
...
def _fixed_single_tvt_split_for_dataset(dataset_name: str) -> Tuple[List[int], List[int], List[int]]:
    """Return the fixed single-TVT split requested by the user.
    ...
    DEAP-A / DEAP-V:
        train = [28, 25, 4, 26, 16, 24, 11, 15, 7, 17,
                 8, 22, 13, 29, 6, 9, 18, 23, 5, 30]
        val   = [12, 20, 3, 1, 21, 31]
        test  = [2, 0, 10, 14, 27, 19]
    """
    key = _dataset_split_key(dataset_name)
    ...
    raise ValueError(
        f'No fixed single-TVT split preset for dataset={dataset_name!r}. '
        'Please provide --train_subjects, --val_subjects, and --test_subjects manually.'
    )


def build_single_tvt_subject_split(args: argparse.Namespace) -> Tuple[List[int], List[int], List[int]]:
    """Build one fixed train/validation/test subject split.

    This is not k-fold and not rotating TVT.  The script runs this one split
    once and then reports the final result.

    By default, this function uses the fixed dataset-specific split requested
    by the user.  Manual command-line subject lists still override the preset.
    """
    num_subjects = int(args.num_subjects)

    manual_train = getattr(args, 'train_subjects', None)
    manual_val = getattr(args, 'val_subjects', None)
    manual_test = getattr(args, 'test_subjects', None)

    if manual_train is not None or manual_val is not None or manual_test is not None:
        if manual_train is None or manual_val is None or manual_test is None:
            raise ValueError(
                'When using manual split, --train_subjects, --val_subjects, '
                'and --test_subjects must all be provided.'
            )
        train_ids = _normalize_subject_ids(manual_train, num_subjects, 'train')
        val_ids = _normalize_subject_ids(manual_val, num_subjects, 'validation')
        test_ids = _normalize_subject_ids(manual_test, num_subjects, 'test')
    else:
        train_ids, val_ids, test_ids = _fixed_single_tvt_split_for_dataset(args.dataset)

        # Validate after selecting the preset, so dataset metadata catches
        # accidental mismatches such as using a SEED preset with DEAP metadata.
        train_ids = _normalize_subject_ids(train_ids, num_subjects, 'train')
        val_ids = _normalize_subject_ids(val_ids, num_subjects, 'validation')
        test_ids = _normalize_subject_ids(test_ids, num_subjects, 'test')
    ...
    if split_sets[0] & split_sets[1] or split_sets[0] & split_sets[2] or split_sets[1] & split_sets[2]:
        raise ValueError(
            f'Train/validation/test splits must be disjoint, got '
            f'train={train_ids}, val={val_ids}, test={test_ids}.'
        )
    if len(split_sets[0] | split_sets[1] | split_sets[2]) != num_subjects:
        raise ValueError(
            f'TVT split must cover all subjects exactly once. '
            f'num_subjects={num_subjects}, train={train_ids}, val={val_ids}, test={test_ids}.'
        )

    return train_ids, val_ids, test_ids


def prepare_data_by_ids(
    args: argparse.Namespace,
    train_ids: List[int],
    validation_ids: List[int],
    test_ids: List[int],
) -> Tuple[Dict[str, Any], Dict[str, Any], Dict[str, Any]]:
    """Build train/validation/test splits from explicit subject-id lists.

    The test split is still used as the unlabeled target domain during
    adaptation. Test labels are kept only for the final evaluation after the
    validation-selected checkpoint has been restored.
    """
    data, label = utils.load_data(
        args.dataset,
        dataset_root=args.dataset_path,
    ...
    train_ids = _normalize_subject_ids(train_ids, num_subjects, 'train')
    validation_ids = _normalize_subject_ids(validation_ids, num_subjects, 'validation')
    test_ids = _normalize_subject_ids(test_ids, num_subjects, 'test')
    ...
    validation_set = {
        'feature': np.ascontiguousarray(
            np.vstack([data_session[i] for i in validation_ids]), dtype=np.float32
        ),
        'label': np.ascontiguousarray(
            np.vstack([label_session[i] for i in validation_ids]), dtype=np.int64
        ),
    }
    target_set = {
        'feature': np.ascontiguousarray(
            np.vstack([data_session[i] for i in test_ids]), dtype=np.float32
        ),
        'label': np.ascontiguousarray(
            np.vstack([label_session[i] for i in test_ids]), dtype=np.int64
        ),
    }
    return target_set, validation_set, source_set
...
def train_epoch(model: nn.Module, domain_discriminator: nn.Module,
    ...
    for batch_idx in range(num_batches):
        ...
        # Keep source/target batch sizes equal for old DAANLoss implementations
        # that split concatenated source+target tensors into two equal chunks.
        if src_data.size(0) != tar_data.size(0):
            common_batch = min(src_data.size(0), tar_data.size(0))
            src_data = src_data[:common_batch]
            src_idx = src_idx[:common_batch]
            src_label = src_label[:common_batch]
            tar_data = tar_data[:common_batch]
            tar_idx = tar_idx[:common_batch]
...
def main_single_tvt(
    ...
    """Train one fixed train/validation/test split.

    This is the non-rotating TVT protocol:
        train_ids      -> source labeled training subjects
        validation_ids -> checkpoint selection only
        test_ids       -> unlabeled target adaptation + final evaluation
    """
    set_seed(args.seed)

    target_set, validation_set, source_set = prepare_data_by_ids(
        args, train_ids, validation_ids, test_ids
    )
    data_loaders, source_sample_num, target_sample_num = create_data_loaders(
        source_set, target_set, validation_set, args
    )
    ...
    os.makedirs(model_dir, exist_ok=True)
    model_path = os.path.join(model_dir, 'train_val_test_best.pth')

    logger.info(
        'Single TVT split: train=%s, validation=%s, test=%s; '
        'test features are used unlabeled for adaptation.',
        train_ids,
        validation_ids,
        test_ids,
    ...
    logger.info(
        'Single TVT sample counts: source=%d, validation=%d, target/test=%d.',
        source_sample_num,
        len(validation_set['feature']),
        target_sample_num,
    )
    ...
    if best_epoch < 0 or not os.path.isfile(model_path):
        raise RuntimeError('No validation checkpoint was saved for this TVT split.')
    ...
    logger.info(
        'Final single TVT test split %s selected by validation split %s: '
        'accuracy=%.4f, loss=%.6f, best_epoch=%d.',
        test_ids,
        validation_ids,
        test_acc,
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        '--protocol', type=str, default='single_tvt',
        choices=['single_tvt', 'strict_loso'],
        help='single_tvt runs one fixed train+val+test split; strict_loso keeps the old N-2+1+1 rotation.',
    )
    parser.add_argument(
        '--tvt_train_ratio', type=float, default=0.6,
        help='legacy option; fixed dataset-specific splits are used by default unless manual subject lists are provided',
    )
    parser.add_argument(
        '--tvt_val_ratio', type=float, default=0.2,
        help='legacy option; fixed dataset-specific splits are used by default unless manual subject lists are provided',
    )
    parser.add_argument(
        '--tvt_test_ratio', type=float, default=0.2,
        help='legacy option; fixed dataset-specific splits are used by default unless manual subject lists are provided',
    )
    parser.add_argument(
        '--tvt_shuffle', action='store_true',
        help='legacy option kept for CLI compatibility; ignored by fixed split preset',
    )
    parser.add_argument(
        '--train_subjects', type=int, nargs='+', default=None,
        help='manual 0-based train subject ids for single_tvt',
    )
    parser.add_argument(
        '--val_subjects', type=int, nargs='+', default=None,
        help='manual 0-based validation subject ids for single_tvt',
    )
    parser.add_argument(
        '--test_subjects', type=int, nargs='+', default=None,
        help='manual 0-based test subject ids for single_tvt',
    )
    ...
    if args.num_subjects < 3:
        raise ValueError('Train/validation/test protocols require at least three subjects.')

    if args.protocol == 'single_tvt':
        train_ids, validation_ids, test_ids = build_single_tvt_subject_split(args)
        logger.info(
            'Starting single non-rotating train + validation + test split.'
        )
        logger.info(
            'Single TVT subject split | train=%s | validation=%s | test=%s',
            train_ids,
            validation_ids,
            test_ids,
        )
        writer_dir = (
            ...
            f'session_{args.session}_single_tvt/'
            f'train_val_test'
        )
        writer = SummaryWriter(writer_dir)
        test_acc, val_acc, best_epoch, conf_matrix = main_single_tvt(
            train_ids,
            validation_ids,
            test_ids,
            writer,
        ...
        result_path = os.path.join(result_dir, 'PCL_TDGCN_single_tvt_results.npy')
        np.save(
            result_path,
            {
        ...
        logger.info('=' * 60)
        logger.info('Single train + validation + test complete')
        logger.info('Final test accuracy: %.4f', test_acc)
        logger.info('Validation-best accuracy: %.4f', val_acc)
        logger.info('Selected epoch: %d', best_epoch)
        logger.info('Final-test confusion matrix: %s', conf_matrix)
        ...
        os.makedirs(result_dir, exist_ok=True)
        result_path = os.path.join(result_dir, 'PCL_TDGCN_strict_loso_results.npy')
        np.save(
            result_path,
            {
                'protocol': args.protocol,
                ...
                'session': args.session,
                'seed': args.seed,
                'test_acc': all_test_accuracies,
                'validation_acc': np.asarray(all_validation_accuracies, dtype=np.float64),
                'best_epoch': np.asarray(all_best_epochs, dtype=np.int64),
