"""
MAET model integrated from the official implementation:
https://github.com/935963004/MAET

The core MAET blocks are preserved. A lightweight LibEER wrapper is added to
adapt EEG tensor input shape (B, C, F) -> (B, C*F) for single-modality EEG use.
"""
...
class Block(nn.Module):
    def __init__(
        ...
        self.mlp_text = Mlp(in_features=dim, hidden_features=mlp_hidden_dim, act_layer=act_layer, drop=drop)
        self.mlp_imag = Mlp(in_features=dim, hidden_features=mlp_hidden_dim, act_layer=act_layer, drop=drop)
        self.mlp_vl = None
        if with_mixffn:
            self.mlp_vl = Mlp(in_features=dim, hidden_features=mlp_hidden_dim, act_layer=act_layer, drop=drop)
    ...
    def forward(self, x, mask=None, modality_type=None, relative_position_bias=None):
        if modality_type == "eeg" and self.eeg_prompt is not None:
            eeg_prompt = self.eeg_prompt.expand(x.size()[0], -1, -1)
            x = torch.cat((x, eeg_prompt), dim=1)
        elif modality_type == "eye" and self.eye_prompt is not None:
            eye_prompt = self.eye_prompt.expand(x.size()[0], -1, -1)
...
class MAET(nn.Module):
    ...
    def forward_features(self, eeg, eye, alpha):
        ...
        if eye is None:
            ...
            x = x + torch.cat(
                [
                    self.eeg_type_embed.expand(x.size()[0], self.eeg_seq_len + 1, -1),
                    self.eye_type_embed.expand(x.size()[0], self.eye_seq_len + 1, -1),
                ],
                dim=1,
            )
            pos_embed = torch.cat(
                [self.eeg_pos_embed.expand(x.size()[0], -1, -1), self.eye_pos_embed.expand(x.size()[0], -1, -1)],
                dim=1,
            )
            x = x + pos_embed
        x = self.pos_drop(x)
...
class MultiViewEmbedding(nn.Module):
    ...
    def forward(self, x):
        ...
        x2 = self.sigmoid(self.transform2(x)).reshape(bsz, self.heads, self.output_dim)
...
class MAETForLibEER(nn.Module):
    def __init__(self, num_electrodes=62, in_channels=5, num_classes=3, domain_generalization=None, num_domains=None):
        super().__init__()
        cfg = self._load_cfg()
        params = cfg.get("params", {})

        # Domain-generalization switch should be decided by training script.
        if domain_generalization is None:
            domain_generalization = False
        if num_domains is None:
            num_domains = int(params.get("num_domains", 19))

        eeg_dim = int(num_electrodes) * int(in_channels)

        self.model = MAET(
            eeg_dim=eeg_dim,
            eye_dim=int(params.get("eye_dim", 33)),
            num_classes=num_classes,
            embed_dim=int(params.get("embed_dim", 32)),
            depth=int(params.get("depth", 3)),
            eeg_seq_len=int(params.get("eeg_seq_len", 5)),
            eye_seq_len=int(params.get("eye_seq_len", 5)),
            num_heads=int(params.get("num_heads", 4)),
            mlp_ratio=float(params.get("mlp_ratio", 4.0)),
            qkv_bias=bool(params.get("qkv_bias", True)),
            ...
            use_abs_pos_emb=bool(params.get("use_abs_pos_emb", False)),
            layer_scale_init_values=float(params.get("layer_scale_init_values", 0.1)),
            mixffn_start_layer_index=int(params.get("mixffn_start_layer_index", 2)),
            use_mean_pooling=bool(params.get("use_mean_pooling", False)),
            domain_generalization=domain_generalization,
            num_domains=num_domains,
            prompt=bool(params.get("prompt", False)),
            prompt_len=int(params.get("prompt_len", 2)),
        )
    ...
    def forward(self, x, alpha_=0):
        if x.ndim > 2:
            x = x.reshape(x.shape[0], -1)
        return self.model(eeg=x, eye=None, alpha_=alpha_)
