...
class DMS_SGPANAblationSpec:
    normalization: str = "subject_invariant"
...
DMS_SGPAN_ABLATION_SPECS = {
    ...
    "standard_norm": replace(BASELINE_ABLATION_SPEC, normalization="source_zscore"),
...
class StandardCrossScaleAttention(nn.Module):
    ...
    def forward(
        ...
        output = self.norm(shared_tokens + shared_ctx)
...
class DMS_SGPAN(nn.Module):
    def __init__(self, net_params: Dict):
        ...
        if self.ablation_spec.normalization == "source_zscore":
        ...
        self.graph_only_encoder = (
            FeatureBlock(self.graph_hidden, self.spectral_hidden, self.dropout)
        ...
        self.basic_encoder = (
            FeatureBlock(flat_dim, self.spectral_hidden, self.dropout)
    ...
    def _encode_primary(self, x_norm: torch.Tensor) -> Dict[str, object]:
        ...
        if self.ablation_spec.encoder == "dual_branch":
            spectral_scales = self._build_spectral_scales(x_norm)
            feature_scales = self._fuse_graph_features(graph_step_feat, spectral_scales)
            ...
            feature_scales = [self.graph_only_encoder(graph_step_feat)]
        elif self.ablation_spec.encoder == "basic_mlp":
            feature_scales = [self.basic_encoder(x_norm.reshape(x_norm.shape[0], -1))]
        else:
    ...
    def _encode_all(self, x: torch.Tensor, subject_ids: torch.Tensor) -> Dict[str, torch.Tensor]:
        ...
        primary = self._encode_primary(x_norm)
        feature_scales = primary["feature_scales"]

        if self.disentanglement_enabled:
            shared_seq_list = [self.shared_encoders[i](feat) for i, feat in enumerate(feature_scales)]
            private_seq_list = [self.private_encoders[i](feat) for i, feat in enumerate(feature_scales)]
            clean_shared_seq = self.cross_scale_attention(shared_seq_list, private_seq_list)
        else:
            clean_shared_seq = [self.direct_encoders[i](feat) for i, feat in enumerate(feature_scales)]
            private_seq_list = []
